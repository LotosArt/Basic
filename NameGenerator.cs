...
public class NameGenerator
{
    ...
    {
        return $"{sampleNames[r.Next(sampleNames.Length)]} {sampleSurnames[r.Next(sampleSurnames.Length)]}";
    }
 
    public string[] GetRandomNames(int count = 10)
    {
        string[] names = new string[10];
        for (int i = 0; i < names.Length; i++)
        {
            names[i] = $"{sampleNames[r.Next(sampleNames.Length)]} {sampleSurnames[r.Next(sampleSurnames.Length)]}";
        }
        return names;
    }
}